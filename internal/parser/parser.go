...
func (p *Parser) parseInterfaceDeclaration() ast.Statement {
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		if p.curToken.Type == token.FN {
			...
			p.errors = append(p.errors, fmt.Sprintf("expected function signature in interface, got %s", p.curToken.Type))
			return iface
		}
...
func (p *Parser) parseConditionStatement() ast.Statement {
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		...
		case token.SEMICOLON:
			// skip
		default:
			p.errors = append(p.errors, fmt.Sprintf("expected 'when' or 'else' in condition block, got %s", p.curToken.Type))
			return stmt
		}
		p.nextToken()
	}
